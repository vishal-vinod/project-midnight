...
import time
import cv2
from data_ops import SonyDataset, SonyTestDataset, iPhone11TrainSet
import torch
...
def grad_loss(gen_frames, gt_frames, alpha=2):
  ...
  gen_dx, gen_dy = grad(gen_frames)
  gt_dx, gt_dy = grad(gt_frames)

  grad_diff_x = gt_dx - gen_dx
  grad_diff_y = gt_dy - gen_dy

  return torch.mean(grad_diff_x ** alpha + grad_diff_y ** alpha)


def train(args, no_of_items, load=False):
    # device
    ...
    target_trainset = iPhone11TrainSet(args.target_input_dir, args.target_gt_dir, args.ps, no_of_items=args.num_target_samples)
    ...
    for epoch in range(args.start_epoch, args.num_epoch):
        ...
        for i, data in enumerate(zip(source_train_loader, cycle(target_train_loader))):
            ...
            target_input_patch, target_gt_patch, target_raw_patch, target_train_id, target_ratio = target_batch #target_train_loader[np.random.random_integers(0, target_len - 1)]
            if epoch == 0:
                logging.info ('[%d] target train id : %d, ratio : %g' % ( i, target_train_id.data.numpy(), target_ratio.data.numpy() ) ) 
            target_input_patch, target_gt_patch, target_raw_patch = target_input_patch.to(device), target_gt_patch.to(device), target_raw_patch.to(device)

            # zero the parameter gradients
            # forward + backward + optimize
            task_optimizer.zero_grad()
            source_outputs, _ = task(source_input_patch, True)
            ...
            # train target data
            task_optimizer.zero_grad()
            target_outputs, jpeg_out = task(target_input_patch, False)
            
            # RAW loss for intermediate output
            target_raw_loss = criterion(target_outputs, target_raw_patch)
            target_raw_loss = target_raw_loss.type(torch.cuda.FloatTensor)
            
            # L1 loss for the compression output
            target_l2_loss = criterion(jpeg_out, target_gt_patch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target_input_dir', type=str, default='/workspace/midnight/dataset/iPhone11/short/')
    parser.add_argument('--target_gt_dir', type=str, default='/workspace/midnight/dataset/iPhone11/long/')
    ...
    parser.add_argument('--model_load_dir2', type=str, default='/workspace/midnight/checkpoints/Raw/Sony_iPhone11/')
    parser.add_argument('--model_to_load2', type=str, default='1500')
    parser.add_argument('--checkpoint_dir', type=str, default='/workspace/midnight/checkpoints/Raw/Sony_iPhone11/')

    parser.add_argument('--result_dir', type=str, default='/workspace/midnight/results/Raw/Sony_iPhone11/')
    parser.add_argument('--val_result_dir', type=str, default='/workspace/midnight/result/Raw/Sony_iPhone11/')
    ...
    logging.basicConfig(level=logging.DEBUG,
        ...
        filename=os.path.join(args.result_dir, 'log_sony_iphone_1.txt'),
        filemode='w')
